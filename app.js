(() => {
  ...
  async function refreshSession() {
    ...
    session = await response.json(); localStorage.setItem(SESSION_KEY, JSON.stringify(session)); return true;
  }
  ...
  function portfolioNumbers(data) {
    ...
    const invested = Number(data?.testAccount?.invested || 0), free = Number(data?.testAccount?.cash || 0), total = Number(data?.testAccount?.total || free + invested);
  ...
  function renderConnected() {
    ...
    $("#engineMessage").textContent = systemRunning ? brokerState.marketOpen ? "Background AI actively monitoring and trading" : "Background AI active · waiting for market open" : "Paper broker connected and ready";
    ...
    $("#cashValue").textContent = money(free); $("#investedValue").textContent = money(invested); $("#todayChange").textContent = `${gain >= 0 ? "+" : ""}${money(gain)}`;
    ...
    $("#analysisText").textContent = systemRunning ? brokerState.marketOpen ? "Automatic secured market cycles are running in the background." : "AI remains active and will trade automatically when the US market opens." : "Connected—press Start when ready";
    ...
    const decisions = brokerState.decisions || [], latest = decisions[0], latestSignals = latest?.signals || {};
  ...
  function renderChart(snapshots) {
    const values = snapshots.map(s => Number(s.value)).filter(Number.isFinite); if (values.length < 2) return;
    const min = Math.min(...values), max = Math.max(...values), range = Math.max(.01, max - min);
    const points = values.map((v, i) => `${(i / (values.length - 1)) * 900},${270 - ((v - min) / range) * 240}`), line = `M${points.join(" L")}`;
    $("#chartLine").setAttribute("d", line); $("#chartArea").setAttribute("d", `${line} L900 300 L0 300 Z`);
    const [x, y] = points.at(-1).split(","); $("#chartPoint").setAttribute("cx", x); $("#chartPoint").setAttribute("cy", y);
  }
  function renderDisconnected(message = "Secure connection setup required") {
    $("#marketStatus").innerHTML = `<i></i> Broker disconnected`; $("#engineState").textContent = "DISCONNECTED"; $("#engineMessage").textContent = message;
    $("#startEngine").disabled = false; $("#stopEngine").disabled = true; $("#analysisText").textContent = "Waiting for Trading 212 Demo and Supabase…"; $("#chartCaption").textContent = "NO MARKET DATA";
  }
  ...
  async function signIn() {
    const email = $("#ownerEmail").value.trim(), password = $("#ownerPassword").value, message = $("#loginMessage"), button = $("#signInButton");
    ...
    try {
      ...
      session = details; localStorage.setItem(SESSION_KEY, JSON.stringify(session)); message.textContent = "Signed in securely."; $("#loginFields").hidden = true; $("#confirmStart").hidden = false; await refresh();
  ...
  $("#startEngine").addEventListener("click", () => {
    ...
    $("#confirmPanel").hidden = false; document.body.style.overflow = "hidden"; $("#loginFields").hidden = Boolean(session); $("#confirmStart").hidden = !session;
  });
  $("#signInButton").addEventListener("click", signIn); $("#ownerPassword").addEventListener("keydown", event => { if (event.key === "Enter") signIn(); });
  $("#confirmStart").addEventListener("click", () => changeEngine("start")); $("#stopEngine").addEventListener("click", () => changeEngine("stop"));
  $("#closeConfirm").addEventListener("click", () => { $("#confirmPanel").hidden = true; document.body.style.overflow = ""; }); $("#resetAccount").addEventListener("click", refresh);
  if (configured() && session) refreshSession().then(ok => ok ? refresh() : renderDisconnected("Please sign in again")); else renderDisconnected();
})();